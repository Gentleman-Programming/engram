...
- `POST /projects/merge` — Merge normalization-equivalent legacy project names into one canonical project. The JSON body is limited to 8 KiB: `{from:["Legacy Name"], to:"canonical name", confirmed:true}`.
  - This administrative route always requires a configured `ENGRAM_HTTP_TOKEN` and matching `Authorization: Bearer <token>`. A missing server token returns `503`; missing or wrong credentials return `401`.
  - `confirmed:true`, at least one source, and a non-blank canonical target are required. Malformed requests and sources that do not normalize to the canonical target return `400`. Every source is validated before mutation, so a mixed valid/invalid list fails without a partial merge.
  - `200` returns `{canonical, sources_merged, observations_updated, sessions_updated, prompts_updated}`. Repeating a valid request is idempotent and returns zero updated records when nothing remains to merge.
  - The route delegates to `Store.MergeProjects`: observations, sessions, prompts, pending sync mutations, and sync enrollment move in one transaction. A successful call notifies autosync, including when the record counters are zero because sync metadata may still have changed.
...
| `ENGRAM_HTTP_TOKEN`             | Optional Bearer auth for the local HTTP server. When set, `DELETE /sessions/{id}`, `DELETE /observations/{id}`, `DELETE /prompts/{id}`, `GET /export`, and `POST /import` require `Authorization: Bearer <token>`. `POST /projects/merge`, `POST /projects/rescue-ownership`, and deprecated alias `POST /projects/migrate` always require a configured token and matching Bearer credential. Comparison is constant-time. Token is read at request time (no restart needed). Other routes remain open when unset (zero-config default). Project consolidation and ownership repair remain available without this token through the local `engram projects consolidate` and `engram projects rescue-ownership` CLI flows. | (unset — HTTP project admin routes not served; local CLI flows still available) |
