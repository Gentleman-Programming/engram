...
func runCodexUserPromptSubmit(input []byte, baseURL, stateDir string, now func() time.Time) []byte {
	...
	key := in.SessionID
	if key == "" { key = "unknown-" + strconv.Itoa(os.Getpid()) }
	loaded, nudge := codexPromptStatePaths(stateDir, key)
	if codexFirstPrompt(loaded) { return codexMessage(codexToolSearchMessage) }
	if !valid || strings.TrimSpace(in.CWD) == "" || baseURL == "" { return []byte("{}") }

	ctx, cancel := context.WithTimeout(context.Background(), codexUserPromptDeadline)
	defer cancel()
	client := &http.Client{}
	var authority json.RawMessage
	if !codexJSON(ctx, client, http.MethodGet, baseURL+"/project/current?cwd="+url.QueryEscape(in.CWD), nil, &authority) { return []byte("{}") }
	project, authorized := codexProjectAuthority(authority)
	if !authorized { return []byte("{}") }
	...
	if knownAge && age < 5*time.Minute { return []byte("{}") }
	last, hasLast, observationsOK := codexLastSave(ctx, client, baseURL, project)
	if !observationsOK { return []byte("{}") }
	elapsed := age
	if hasLast { elapsed = now().Sub(last) }
	if (!hasLast && !knownAge) || elapsed < 15*time.Minute || !codexNudgeAllowed(nudge, now()) { return []byte("{}") }
...
func codexHookURL() string {
	port := strings.TrimSpace(os.Getenv("ENGRAM_PORT")); if port == "" { port = "7437" }
	n, err := strconv.Atoi(port); if err != nil || n < 1 || n > 65535 { return "" }
	return "http://127.0.0.1:" + port
}
func codexPromptStatePaths(dir, key string) (string, string) {
	sum := sha256.Sum256([]byte(key)); name := "engram-codex-" + hex.EncodeToString(sum[:])
	return filepath.Join(dir, name+"-tools-loaded"), filepath.Join(dir, name+"-last-nudge")
}
func codexFirstPrompt(path string) bool {
	if os.MkdirAll(filepath.Dir(path), 0o700) != nil { return true }
	file, err := os.OpenFile(path, os.O_CREATE|os.O_EXCL|os.O_WRONLY, 0o600)
	if err == nil { _ = file.Close(); return true }
	return !os.IsExist(err)
}
func codexProjectAuthority(raw json.RawMessage) (string, bool) {
	var fields map[string]json.RawMessage
	if json.Unmarshal(raw, &fields) != nil || fields == nil { return "", false }
	if _, hasErrorHint := fields["error_hint"]; hasErrorHint { return "", false }
	var project, source string
	if json.Unmarshal(fields["project"], &project) != nil || strings.TrimSpace(project) == "" ||
		json.Unmarshal(fields["project_source"], &source) != nil || !codexProjectSource(source) { return "", false }
	return project, true
}
func codexProjectSource(source string) bool {
	for _, allowed := range []string{"config", "git_remote", "git_root", "git_child", "dir_basename", "process_override"} { if source == allowed { return true } }
	return false
}
func codexSessionAge(ctx context.Context, c *http.Client, baseURL, sessionID string, now time.Time) (time.Duration, bool) {
	if sessionID == "" { return 0, false }
	var response struct { StartedAt string `json:"started_at"` }
	if !codexJSON(ctx, c, http.MethodGet, baseURL+"/sessions/"+url.PathEscape(sessionID), nil, &response) { return 0, false }
	started, ok := codexHookTime(response.StartedAt); return now.Sub(started), ok
}
func codexLastSave(ctx context.Context, c *http.Client, baseURL, project string) (time.Time, bool, bool) {
	var response []struct { CreatedAt string `json:"created_at"` }
	path := baseURL + "/observations?project=" + url.QueryEscape(project) + "&limit=1&sort=created_at:desc"
	if !codexJSON(ctx, c, http.MethodGet, path, nil, &response) { return time.Time{}, false, false }
	if len(response) == 0 { return time.Time{}, false, true }
	last, ok := codexHookTime(response[0].CreatedAt); return last, ok, ok
}
func codexHookTime(value string) (time.Time, bool) {
	for _, layout := range []string{time.RFC3339Nano, time.RFC3339, "2006-01-02 15:04:05"} {
		parsed, err := time.Parse(layout, strings.TrimSpace(value)); if err == nil { return parsed, true }
	}
	return time.Time{}, false
}
func codexJSON(ctx context.Context, c *http.Client, method, endpoint string, body []byte, target any) bool {
	request, err := http.NewRequestWithContext(ctx, method, endpoint, strings.NewReader(string(body))); if err != nil { return false }
	if body != nil { request.Header.Set("Content-Type", "application/json") }
	response, err := c.Do(request); if err != nil || response == nil { return false }; defer func() { _ = response.Body.Close() }()
...
func codexNudgeAllowed(path string, now time.Time) bool {
	...
	if seconds, err := strconv.Atoi(os.Getenv("ENGRAM_NUDGE_COOLDOWN_SECS")); err == nil && seconds >= 0 { cooldown = time.Duration(seconds) * time.Second }
	if last, err := os.ReadFile(path); err == nil {
		if epoch, err := strconv.ParseInt(strings.TrimSpace(string(last)), 10, 64); err == nil && now.Sub(time.Unix(epoch, 0)) < cooldown { return false }
	}
	return os.WriteFile(path, []byte(strconv.FormatInt(now.Unix(), 10)), 0o600) == nil
}
func codexMessage(message string) []byte { output, _ := json.Marshal(map[string]string{"systemMessage": message}); return output }
