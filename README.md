...
| `engram context [project] [--project NAME\|--all]` | Recent session context; positional project remains supported |
...
| `ENGRAM_PROJECT`                | Process-level default project override, applied by project-scoped entry points. Precedence: explicit request project (`engram save --project`, MCP tool `project` argument) → process override (`engram mcp --project`, then `ENGRAM_PROJECT`) → cwd detection. It must be a project name, not a path. Project-scoped reads use this current project when omitted; use `--all` or `all_projects=true` for a deliberate global read. | cwd-detected project |
