...
func TestDeleteProject_HardDeleteCascadeAndSyncCleanup(t *testing.T) {
	...
	result, err := s.DeleteProject(project, true)
	if err != nil {
	...
	for _, q := range []string{
		...
		fmt.Sprintf("SELECT COUNT(*) FROM sync_chunks WHERE target_key = '%s'", projectTargetKey),
		fmt.Sprintf("SELECT COUNT(*) FROM sync_state WHERE target_key = '%s'", projectTargetKey),
...
func TestDeleteProject_PreservesGlobalPendingSyncStateForOtherProjects(t *testing.T) {
	...
	if _, err := s.DeleteProject("beta", true); err != nil {
...
func TestDeleteProject_NoOpForUnknownProject(t *testing.T) {
	...
	_, err := s.DeleteProject("does-not-exist", true)
	if !errors.Is(err, ErrProjectNotFound) {
		t.Fatalf("expected ErrProjectNotFound for unknown project, got %v", err)
	}
}
...
func TestDeleteProject_DeletesHistoricalProjectNameVariants(t *testing.T) {
	...
	result, err := s.DeleteProject("engram-memory", true)
	if err != nil {
		t.Fatalf("DeleteProject: %v", err)
	}
	if !result.Deleted {
