#!/bin/bash
# Engram — UserPromptSubmit hook for Droid
#
# On the FIRST message of a session: injects a ToolSearch instruction to force
# Droid to load all engram memory tools (which are deferred by default).
#
...
# this, an agent that genuinely has nothing to save never resets the last-save
# clock, so the reminder would fire on every single message forever.
#
# MUST exit 0 always and output valid JSON — otherwise Droid blocks the message.

ENGRAM_PORT="${ENGRAM_PORT:-7437}"
ENGRAM_URL="http://127.0.0.1:${ENGRAM_PORT}"

# Windows Git Bash/MSYS2 can fail while forking helper processes under
# enterprise Defender/EDR, which makes Droid wait on prompt submission.
...
if is_windows_bash && [ "${ENGRAM_DROID_WINDOWS_BASH_SAFE_MODE:-auto}" != "0" ]; then
  ...
  if [ -n "$SESSION_ID" ]; then
    ...
    SESSION_KEY="engram-droid-${JSON_VALUE}-tools-loaded"
  else
    SESSION_KEY="engram-droid-windows-$$-tools-loaded"
  fi
...
if [ -n "$SESSION_ID" ]; then
  SESSION_KEY="engram-droid-${SESSION_ID}-tools-loaded"
else
  ...
  SESSION_KEY="engram-droid-${SAFE_PROJECT}-$$-tools-loaded"
fi
