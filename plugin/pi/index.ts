...
// Writes have a short, single-attempt deadline because a response timeout leaves their
// server-side outcome unknown. Read-only diagnostics get a longer, bounded retry budget.
...
// These codes establish that the request could not leave the client. Keep this list narrow:
// resets and other socket failures can occur after dispatch and must retain an unknown outcome.
const DEFINITE_PRE_DISPATCH_CODES = new Set([
  "ENOTFOUND",
  "EAI_AGAIN",
  "ENETUNREACH",
  "EHOSTUNREACH",
...
async function engramFetchResult<TResponse = unknown>(path: string, opts: FetchOptions = {}): Promise<EngramFetchResult<TResponse>> {
  ...
  if (!res) {
    if ((timeout || ambiguousTransport) && (policy.operation === "write" || policy.operation === "session-registration")) {
      return { data: null, transportFailure: { operation: policy.operation, outcome: "unknown", timeoutMs: policy.timeoutMs } };
    }
    if (timeout) return { data: null, transportFailure: { operation: policy.operation, outcome: "timed_out", timeoutMs: policy.timeoutMs } };
...
async function bestEffortEngramFetch<TResponse = unknown>(path: string, opts: FetchOptions = {}): Promise<TResponse | null> {
  try {
    ...
    if (result.transportFailure) {
      warnEngramFailure(path, new Error(unreachableMessage(result.transportFailure)));
    }
    return result.data;
  } catch (error) {
...
function unreachableMessage(transportFailure: EngramTransportFailure | undefined): string {
  if (transportFailure?.operation === "session-registration" && transportFailure.outcome === "unknown") {
    return `gentle-engram could not confirm session registration after ${transportFailure.timeoutMs}ms. Registration is idempotent and was retried within its bounded policy, but its final outcome is unknown. No memory write was sent; retrying the memory operation is safe.`;
  }
  if (transportFailure?.operation === "write" && transportFailure.outcome === "unknown") {
    return `gentle-engram could not confirm the write after ${transportFailure.timeoutMs}ms. Its outcome is unknown because the server may already have applied it — do NOT blindly retry it, or you may duplicate the write. Verify with mem_search or mem_doctor first.`;
  }
  if (transportFailure?.outcome === "timed_out") {
    return `gentle-engram ${transportFailure.operation} timed out after ${transportFailure.timeoutMs}ms waiting for the Engram HTTP server at ${ENGRAM_URL}. The bounded read policy was exhausted; run mem_doctor or retry the read when the server is healthy.`;
  }
...
async function executeMemoryTool(toolName: string, params: Record<string, unknown>, ctx: MemoryToolContext, signal?: AbortSignal) {
  ...
  try {
    ...
    const transportFailure = transport.transportFailure();
    if (transportFailure) throw new Error(unreachableMessage(transportFailure));
    ...
    const transportFailure = transport.transportFailure();
    const message = transportFailure ? unreachableMessage(transportFailure) : error instanceof Error ? error.message : String(error);
    const details = error instanceof EngramHttpError
      ? { error: message, http_status: error.status, data: error.data }
      : transportFailure
        ? { error: message, outcome: transportFailure.outcome, operation: transportFailure.operation }
        : { error: message };
