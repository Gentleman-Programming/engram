...
func TestCodexWindowsNativeUserPromptTrustedPinForwardsInputAndMeetsTimingBudget(t *testing.T) {
	...
	if posts != 1 {
		t.Fatalf("prompt posts=%d, want at-most-once dispatch", posts)
	}
	...
	if posts != 13 || firstP95 >= 1500*time.Millisecond || subsequentP95 >= 1500*time.Millisecond {
		t.Fatalf("posts=%d p95 first=%s subsequent=%s, want 13 posts and both p95 values <1.5s", posts, firstP95, subsequentP95)
	}
}
