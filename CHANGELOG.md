...
- **fix(doctor):** report grouped `orphaned_observation_session` findings when active or soft-deleted observations reference a missing local session. Findings carry the grouped project, session ID, and observation count so the affected reference can be reviewed before any repair.
- **feat(doctor):** add `engram doctor repair --check orphaned_observation_session --project <project> --plan|--dry-run|--apply`. The repair rebuilds one placeholder parent session per orphan group — `directory=''`, `started_at` at the group's earliest observation timestamp (soft-deleted observations included), `ended_at` set to the same value so the rebuilt session is terminal and can never become an active runtime session, `ownership_mode` `project_owned` only for a `manual-save-{project}` ID, and a `placeholder rebuilt by doctor repair (orphaned_observation_session)` summary. It cannot recover the original session metadata; it restores referential integrity so reads and replays stop failing. Groups whose session ID holds an active delete tombstone are skipped (`session_delete_tombstoned`), both in the plan and defensively inside the apply transaction, because rebuilding them would resurrect deliberately deleted data. The repair is local-only — it enqueues no sync mutations and touches no sync cursor — creates a SQLite backup before writing, and reports `noop` when nothing is left to rebuild.
...
- **feat(pi):** add `mem_list_projects`, `mem_pin`, and `mem_unpin` Pi-native tools, bringing the plugin to tool parity with the Engram agent MCP profile. `mem_list_projects` enumerates known projects through the new `GET /projects` route; `mem_pin`/`mem_unpin` toggle local observation pin state through the existing pin routes.
...
- **feat(server):** add `GET /projects`, the HTTP view of `mem_list_projects` backed by the same `ListProjectsWithStats` store query as the MCP tool and `engram projects list`, so CLI, MCP, and HTTP clients never diverge. An empty store answers `200` with an empty listing.
